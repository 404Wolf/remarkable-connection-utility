...
def __win32_finddll():
    ...
    adir = Path(__file__).parent.parent.parent.parent
    gsdir = Path(adir / 'gsdll64.dll')  # see: Make-win.bat
    if gsdir.exists():
        return str(gsdir)
...
if sys.platform == 'win32':
    ...
    print('USING libgs', libgs)
    libgs = windll.LoadLibrary(libgs)
else:
    import ctypes.util
    # This should find gslib.dylib on macOS (automatically included 
    # from PyInstaller).
